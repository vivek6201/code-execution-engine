...
func SetStatus(ctx context.Context, client *redis.Client, id string, status result.Status) error {
...
func SetResult(ctx context.Context, client *redis.Client, id string, r result.Result) error {
	bytes, err := json.Marshal(r)
	if err != nil {
		return err
	}
...
func GetResult(ctx context.Context, client *redis.Client, id string) (*result.Result, error) {
